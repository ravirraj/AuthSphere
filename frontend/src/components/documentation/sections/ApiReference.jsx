...
const ApiReference = () => {
    const sdkMethods = [
        {
            ...
            params: [
                { name: "publicKey", type: "string", desc: "Your project's public key (Client ID)." },
                { name: "baseUrl", type: "string", desc: "The AuthSphere API root URL." },
                { name: "redirectUri", type: "string", desc: "Whitelist callback URL for OIDC redirects." }
            ]
        },
        {
            ...
            throws: "403 VERIFICATION_REQUIRED"
        },
        {
            name: "loginWithProvider(name)",
            desc: "Triggers the OIDC redirect flow for social providers.",
            params: [{ name: "name", type: "'google'|'github'", desc: "Provider identifier." }]
        },
        {
            name: "handleCallback()",
    ...
    const endpoints = [
        ...
        },
        {
            ...
            body: { email: "string", otp: "string", transactionId: "string?" }
        },
        {
            method: "POST",
            path: "/sdk/refresh",
        ...
        <article className="space-y-12 animate-in fade-in duration-500">
            <div className="space-y-3">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed max-w-3xl">
                    Comprehensive technical reference for the AuthSphere SDK and underlying REST endpoints. 
                    All responses follow the standard JSON:API specification.
                </p>
            </div>
                ...
                <div className="rounded-2xl border bg-card overflow-hidden">
                    <Table>
                        <TableHeader className="bg-muted/50">
                            <TableRow>
                                <TableHead className="w-[200px] text-[11px] font-bold uppercase">Method</TableHead>
                                <TableHead className="text-[11px] font-bold uppercase">Description</TableHead>
                                <TableHead className="text-[11px] font-bold uppercase text-right">Returns</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {sdkMethods.map((method, i) => (
                                <TableRow key={i} className="hover:bg-muted/20 transition-colors">
                                    <TableCell className="font-mono text-xs font-bold text-primary">{method.name}</TableCell>
                                    ...
                                    <TableCell className="text-right font-mono text-[10px] text-foreground/80">{method.returns || 'void'}</TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                    </Table>
                    ...
                    {endpoints.map((ep, i) => (
                        <div key={i} className="grid lg:grid-cols-5 gap-8 items-start">
                            <div className="lg:col-span-2 space-y-4">
                                <div className="flex items-center gap-2">
                                    <Badge variant="secondary" className="bg-emerald-500/10 text-emerald-500 border-none px-2 py-0.5 font-bold uppercase text-[10px]">
                                        {ep.method}
                                    </Badge>
                                ...
                                {ep.headers && (
                                    <div className="space-y-2">
                                        <h4 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Required Headers</h4>
                                        <div className="space-y-1">
                                            {ep.headers.map((h, j) => (
                                                <code key={j} className="block text-[10px] bg-muted p-2 rounded border border-transparent hover:border-primary/10 transition-colors">
                                                    {h}
                                                </code>
                                            ))}
                                        </div>
                            ...
                            <div className="lg:col-span-3">
                                <h4 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-3">Sample Payload</h4>
                                <DocsCodeBlock
                                    id={`ep-${i}`}
